...
class Cow(object):
    def __init__(self, retry_policy=QiniuRetryPolicy()):
        self.access_key = app_config.qiniu.access_key
        self.secret_key = app_config.qiniu.secret_key
        self.upload_tokens = {}
    ...
    def put(self, scope, data, filename=None, keep_name=True, override=True, content_type=None):
        ...
        a = requests.Request(url='%s/upload' % UP_HOST, files=files, data=data)
        prepare = a.prepare()
    ...
    def _upload(self, body, content_type):
        ...
        response = yield http.fetch('%s/upload' % UP_HOST,
